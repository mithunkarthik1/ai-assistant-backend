...
async def store_documents(
    ...
    """
    Stores a batch of LangChain Document chunks into PostgreSQL with real dense semantic vectors.
    """
    if not documents:
        return []
    ...
    logger.info("Stored %d chunks with %d-dimensional dense vectors in PostgreSQL.", len(doc_ids), len(vectors[0]) if vectors else 0)
    return doc_ids
